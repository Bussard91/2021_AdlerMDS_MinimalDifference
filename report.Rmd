...
# Inputs for plot x-axis
  ...
  # Linear regression plot
    a <- params$input_Cut
    plot(df$Mean, df$MD, xlab = paste(params$input_Name, "in", params$input_Unit), ylab = paste("Minimal Difference in", params$input_Unit),
         col = "blue", cex = 2, pch = 16, xlim = c(params$input_xaxismin2, params$input_xaxismax2), ylim = c(0, params$input_yaxismax2),
         main = paste("Regression plot of", params$input_Name, "MD based on quality controls"))
    abline(Model, lwd = 2)
    points(x = params$input_Cut, y = Model$coefficients[2]*params$input_Cut+Model$coefficients[1], col = "red", pch = 16, cex = 2)
    legend(x = (params$input_xaxismin2 + params$input_xaxismax2)/1.7, y = params$input_yaxismax2/5, legend = c("Quality controls", "Cut-Off"),
